...
const fetchSearchAnimix = async ({ list = [], keyw }) => {
  try {
    if (!keyw)
      return {
        error: true,
    ...
    const fetchAnimix = await axiosPg.request({
...
const fetchAnimixRecentEpisodes = async ({ list = [] }) => {
  try {
    const res = await axiosPg.get(animixBase + 'rsssub.xml')
...
const fetchAnimixAllAnime = async ({ list = [] }) => {
  try {
    const fetchAnimixAll = await axiosPg.get(animixAll, headerOption)
...
const fetchAnimixAnimeInfo = async ({ malId, list = {} }) => {
  try {
    ...
    const fetchInfo = await axiosPg.get(
      animixBase + `assets/mal/${malId}.json`,
      headerOption,
    )
    const fetchInfoLinks = await axiosPg
...
const fetchAnimixEpisodeInfo = async ({ animeId, list = {} }) => {
  try {
    if (!animeId) {
      return {
    ...
    const res = await axiosPg.get(animixBase + `v1/${animeId}`, headerOption)
    let episodes = []
    const $ = load(res.data)
...
const fetchAnimixEpisodeSource = async ({ episodeId }) => {
  try {
    let sources = []
    let type
    let episodeGogoLink
    ...
    let url = animixBase + `v1/${animeId}`
    console.log({url: animixBase + `v1/${animeId}`})

    const res = await cloudscraper.get(url, headerOption)
    const $ = load(res)
    const epList = JSON.parse($('#epslistplace').text())
