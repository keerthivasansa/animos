...
export async function fetchAnimepaheInfo({
  animeId,
  page = 1,
}: {
  animeId: string | null;
  ...
  if (!animeId) throw new Error("No animeId was provided");
  const res = await proxyAxios.get(`${animepaheBase}/a/${animeId}`, {});
  const originalId = res.request.path.split("/")[2] as string;
  ...
  const rangedEpisodes = firstEp.episode2
    ? Number(firstEp.episode2) - Number(firstEp.episode)
    : -1; // for animes like saiki where each "episode" contains multiple episodes.
  epList.data.data.map(
    (ep: {
      episode: number;
      episode2?: number;
      session: string;
...
export const writeEpisodeSource = async (episode: {
  animePaheId: string | null;
  length: number | null;
  anime: { animePaheId: string | null };
}) => {
  ...
  try {
    const resp = await proxyAxios.get(
      `${animepaheBase}/play/${episode.anime.animePaheId}/${episode.animePaheId}`
    );
    ...
    $("#resolutionMenu")
      .children()
      .each((_, elem) => {
        const referrer = $(elem).attr("data-src")!;
        const resolution = parseInt($(elem).attr("data-resolution")!);
        const audio = $(elem).attr("data-audio") as "jpn" | "eng";
        sources.push({ referrer, resolution, audio });
      });
    const res = await Promise.all(
      sources
        .filter((src) => src.audio != "eng")
        .map(async (link) => {
