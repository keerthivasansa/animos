...
export async function search(
    ...
    let resp = await fetch(searchUrl + "?" + searchParams.toString());
    const data = await resp.json();
    if (data.meta.count == 0) return { data: [], totalItems: 0, currentPage: 1 };
    let categorieObjs = data.included.filter((obj: { type: string; }) => obj.type == "categories");
    console.log(categorieObjs[0]);
    let result = data.data.filter(
        (anime: { relationships: { mappings: { data: string | any[]; }; }; }) => anime.relationships.mappings.data.length > 0
    ).map((anime: Record<string, any>) => {
        let t_anime = transformKitsuToAnime(anime);
        let categoryIds = anime.relationships.categories.data.map((cat: { id: any; }) => cat.id);
        console.log(categoryIds);
        let categories = categoryIds.map(
            (cat: any) => categorieObjs.filter((obj: { id: any; }) => obj.id == cat)[0].attributes.title
        );
